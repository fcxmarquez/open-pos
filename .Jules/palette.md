...
## 2026-03-08 - Multi-Input Field Accessibility
**Learning:** For multi-input fields like PINs or OTPs that split a single logical input across multiple elements, screen readers need explicit labels for each individual input to understand its sequence and purpose.
**Action:** Always add explicit `aria-label` attributes to each individual input in multi-input components (e.g., 'Dígito 1 del PIN').
